    ...
    def __init__(self, bounds,pop_size,popsizenow):
        ...
        if popsizenow < (pop_size/3):
                for i in range(0, self.noOfArgs):
                        ...
                        if i == 0:
                                self.x.append(random.randint(bounds[i][0], int(bounds[i][0]+dinum))*1024)
                        else:
                ...
                if popsizenow < (pop_size/3)*2:
                        for i in range(0, self.noOfArgs):
                                dinum = (bounds[i][1] - bounds[i][0])/3
                                if i == 0:
                                        self.x.append(random.randint(int(bounds[i][0]+dinum), int(bounds[i][0]+2*dinum))*1024)
                                else:
                        ...
                        for i in range(0, self.noOfArgs):
                                dinum = (bounds[i][1] - bounds[i][0])/3
                                if i == 0:
                                        self.x.append(random.randint(int(bounds[i][0]+2*dinum),int(bounds[i][1]))*1024)
                                else:
